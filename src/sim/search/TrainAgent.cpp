...
int search::TrainAgent::getIncomingDamage(const BattleContext &bc) const {
...
fixed_list<int,16> search::TrainAgent::getBestMapPathForWeights(const Map &m, const int *weights) {
    Path paths1[7];
    Path paths2[7];
...
void search::TrainAgent::takeAction(BattleContext &bc, search::Action a) {
    actionHistory.emplace_back(a.bits);
    if (print) {
        printHelper(bc, a);
    }
...
void search::TrainAgent::playout(GameContext &gc) {
    curGameContext = &gc;
...
bool search::TrainAgent::playPotion(BattleContext &bc) {
    bool usedAll = true;
    int i = 0;
...
void search::TrainAgent::playoutBattle(BattleContext &bc) {
...
void search::TrainAgent::stepBattleCardPlay(BattleContext &bc) {
...
void search::TrainAgent::stepBattleCardSelect(BattleContext &bc) {
...
void search::TrainAgent::stepOutOfCombat(GameContext &gc) {
...
void search::TrainAgent::stepEventScreen(GameContext &gc) {
    switch (gc.curEvent) {
...
void search::TrainAgent::stepRestScreen(GameContext &gc) {
...
void search::TrainAgent::stepRewardsScreen(GameContext &gc) {
...
void search::TrainAgent::stepCardReward(GameContext &gc) {
...
void search::TrainAgent::stepShopScreen(GameContext &gc) {
    auto &s = gc.info.shop;
...
void agentMtRunner(TrainAgentInfo *info) {
    std::uint64_t seed;
    {
...
void search::TrainAgent::runAgentsMt(int threadCount, std::uint64_t startSeed, int playoutCount, bool print) {
